...
PATCH_FILE = (
    ...
    / "stage1-supervision-patch-full-v2.csv"
)
...
def main():
    ...
    with PATCH_FILE.open(encoding="utf-8", newline="") as f:
        for r in csv.DictReader(f):
            cid = r["canonical_train_id"]
            label = r["label"]
            app = r["applicability"].strip().upper()
    ...
    counts = Counter(
        r["applicability"].strip().upper()
        for r in patch_rows
    )

    # Guards from frozen mapped patch
    assert len(patch_rows) == 1256
    assert len(patch) == 213
    assert counts["NO"] == 988
    assert counts["PLAUSIBLE"] == 123
    assert counts["CLEAR"] == 145
