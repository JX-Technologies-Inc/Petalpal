...
def main():
 out=Path(os.environ.get("CONTINUATION_OUT",HERE/"experiment"));out.mkdir(exist_ok=False);lock=json.loads((PARENT/"annotation-lock.json").read_text());train=rows(PARENT/"train.jsonl");dev=rows(PARENT/"dev.jsonl")
 if os.environ.get("EXTRA_TRAIN"):train+=rows(Path(os.environ["EXTRA_TRAIN"]))
 ...
 cp=Path(os.environ.get("CONTINUATION_INIT",PARENT/"experiment"/"epoch-2-checkpoint"));tok=AutoTokenizer.from_pretrained(cp,local_files_only=True);model=AutoModelForSequenceClassification.from_pretrained(cp,local_files_only=True);pos=label_positions(model)
 ...
 prior_path=Path(os.environ.get("CONTINUATION_PRIOR_SUMMARY",PARENT/"experiment"/"summary.json"));prob_path=Path(os.environ.get("CONTINUATION_PRIOR_PROBS",PARENT/"experiment"/"epoch-2-probabilities.npy"));prior=json.loads(prior_path.read_text());p0=np.load(prob_path);base=prior["finalMetrics"] if "finalMetrics" in prior and os.environ.get("CONTINUATION_PRIOR_SUMMARY") else prior["records"][2]["metrics"];rec=[{"epoch":0,"trainLoss":None,"metrics":base}]
 t0=time.time();opt=torch.optim.AdamW(model.parameters(),lr=2e-6,weight_decay=.01);steps=math.ceil(len(tr)/ACCUM)*2;sched=get_linear_schedule_with_warmup(opt,math.ceil(.1*steps),steps)
 for epoch in [1,2]:
 ...
 eligible=[r for r in rec if r["metrics"]["selected18"]["micro"]["precision"]>=.5];selected=max(eligible,key=lambda r:(r["metrics"]["selected18"]["macro"]["f1"],-r["epoch"]));blend=None;candidates=[("epoch",selected["metrics"])]
 if selected["epoch"]>0:
